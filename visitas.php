...
Description: Registra las visitas a tu p&aacute;gina y lo muestra junto al Copyright.
Version: 1.4 (30/09/2006)
...
global $vr_lang;
	if ($_POST["validar"] == "opciones") {
		...
		if (!isset($si) && ($_POST['vr_days'] > 0) && ($_POST['vr_delay'] > 0) && ($_POST['vr_numof'] > 0) 
...
<div class="wrap">
	...
	<?php
		...
		function borrarSpam () {
			...
			$contador = $wpdb->get_row("SELECT COUNT(*) AS count FROM $wpdb->comments WHERE comment_approved = 'SPAM'");
			$wpdb->query("DELETE FROM $wpdb->comments WHERE comment_approved = 'SPAM'");
			$wpdb->query("OPTIMIZE TABLE $wpdb->comments");
			if (!mysql_error())	{
				$msg = $vr_lang['spam_msg_ok'] . $contador->count . $vr_lang['spam_msg_blog'];
			} else {
				$msg = $vr_lang['msg_err_sql'] . mysql_error();
			}
			...
			$contador = $wpdb->get_row("SELECT COUNT(*) AS count FROM $tbl_visitas WHERE ".$spam);
			$wpdb->query("DELETE FROM $tbl_visitas WHERE".$spam);
			$wpdb->query("OPTIMIZE TABLE $tbl_visitas");
		
			$valor = get_option('vr_old');
			$valor = $valor + $contador->count;
			update_option('vr_old', $valor);
				
			if (!mysql_error())	{
				$msg2 = $vr_lang['spam_msg_ok'] . $contador->count . $vr_lang['spam_msg_stats'];
				$msg3 = $vr_lang['spam_msg_count'] . $valor . ".";
			} else {
				$msg = $vr_lang['msg_err_sql'] . mysql_error();
			}
		?>
		<fieldset class="options">
			...
			<legend><?php echo $vr_lang['spam_frm_count']; ?></legend>
				<div id="message" class="updated fade"><p><?php echo $msg3; ?></p></div>
		</fieldset>	
		<?php
		}
	borrarSpam();
...
// Crea la lista de resultados para los cuadros de estadísticas
// ------------------------------------------------------------
